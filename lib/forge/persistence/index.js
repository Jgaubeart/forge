...
import { createAgentRunStore, findRunForTask } from "./runs.js";
...
export async function getForgePersistence() {
  ...
  return {
    ...
    workspaces: {
      ...
      createForUser: (input) =>
        createWorkspaceForUser(userClient, { ...input, trustedClient }),
    },
    missions: {
    ...
    runs: {
      // The store the single-agent execution layer talks to.
      store: () => createAgentRunStore(clients),
      forMission: (missionId) => findRunForTask({ userClient }, missionId),
    },
    agents: {
