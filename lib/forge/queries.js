...
export async function loadWork(supabase, workspaceId, { limit = 100 } = {}) {
  ...
  const tasksResult = await read(
    supabase
      .from("tasks")
      .select(
        "id, workspace_id, agent_id, status, action_level, input, created_at, updated_at, requested_by, current_step, started_at, completed_at, cancelled_at, cancel_requested_at"
      )
