...
export async function listAgents(supabase) {
  const agentsResult = await read(
    supabase
      .from("agents")
      .select(
        "id, name, slug, description, instructions, is_active, department_id, created_at, departments ( id, name, slug )"
      )
...
export async function loadWork(supabase, workspaceId, { limit = 100 } = {}) {
  ...
  const tasksResult = await read(
    supabase
      .from("tasks")
      .select(
        "id, workspace_id, agent_id, status, action_level, input, created_at, updated_at, requested_by"
      )
  ...
  const runsResult = taskIds.length
    ? await read(
        supabase
          .from("agent_runs")
          .select(
            "id, task_id, hermes_run_id, status, started_at, completed_at, created_at, error"
          )
          .in("task_id", taskIds)
  ...
  const approvalsResult = taskIds.length
    ? await read(
        supabase
          .from("approvals")
          .select(
            "id, task_id, agent_run_id, capability, action_payload, status, expires_at, decided_at, created_at, requested_by"
          )
          .in("task_id", taskIds)
          .order("created_at", { ascending: false }),
        "approvals"
