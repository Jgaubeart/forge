...
export function createFakeRuntimeAdapter({
  ...
  const adapter = {
    ...
    // Phase 6 never creates a run, so the default stays a refusal. Phase 7 tests
    // opt in with `createRun: { id, status }`, or ask for a specific failure with
    // `createRun: { failure: "runtime_unauthorized" }`.
    async createRun(request) {
      calls.push("createRun");
      createRequests.push(request);

      if (!createRun) unavailable();
      if (createRun.failure) {
        throw runtimeError(createRun.failure);
      }

      const run = normalizeRun({
        id: createRun.id ?? `run-${known.size + 1}`,
        status: createRun.status ?? "running",
